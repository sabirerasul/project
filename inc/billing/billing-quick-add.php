...
function addBilling($db, $billing)
{
    ...
    $domain = str_replace("inc/billing/billing-quick-add.php", "", $domain);
...
function addBillingProduct($db, $billing_product, $billing_id)
{
    ...
    foreach ($billing_product as $key => $value) {
        ...
        foreach ($sp_id as $spkey => $sp_id_value) {
            ...
            $sql2 = "INSERT INTO `client_billing_assign_service_provider`(`billing_id`, `billing_service_id`, `service_provider_id`) VALUES ('{$billing_id}','{$billing_service_id}','{$serviceProviderId}')";
