...
function updateServiceProvider($db, $v, $k, $val, $em_photo, $em_frontproof, $em_backproof, $id){

    foreach ($v as $key => $value) {
        if($key != 'service_provider_submit' && $key != 'confirm_password' && $key != 'photo' && $key != 'frontproof' && $key != 'backproof' && $key != 'eid'){
            $k .=  "{$key}='{$value}',";
        }
    }    
    $k = rtrim($k, ',');
    ...
    $sql = "UPDATE `service_provider` SET {$k1} WHERE id={$id}";
...
if (!empty($errors)) {
    ...
    $m = updateServiceProvider($db, $v, $k, $val, $em_photo, $em_frontproof, $em_backproof, $id);
