...
function sendSMTPMail($email, $name, $body, $subject, $imageArr)
{
    ...
    $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
    $mail->Username   = 'noreply@pxlsoftware.com';               //SMTP username
    $mail->Password   = ';^OOU?oyqE8K';                          //SMTP password
    ...
    $pa = dirname(__FILE__);
    $pieces = explode(DIRECTORY_SEPARATOR, $pa);
    $removed1 = array_pop($pieces);
    $removed2 = array_pop($pieces);

    $path1 = implode('/',$pieces);

    $branchLogo = $path1 . "/web/salon-logo/" . $imageArr['filename'];
