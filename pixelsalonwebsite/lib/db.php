...
{
  $productModel = fetch_object($db, "SELECT sum(credit) as creditStock, sum(debit) as debitStock FROM stock_record WHERE stock_main_id='" . $id . "'");
...
function get_login_user_model($db)
{
  $model = fetch_object($db, "SELECT * FROM `user` WHERE user_role='superadmin'");
  return $model;
}

function get_login_user_branch($db)
{
  $branchModel = fetch_object($db, "SELECT * FROM branch");
  return $branchModel;
}
...
function get_salon_timing($db)
{
  ...
  $branch_working_hours_start = $branchModel->working_hours_start;
  $branch_working_hours_end = $branchModel->working_hours_end;
